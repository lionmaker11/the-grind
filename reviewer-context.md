...
- 2026-06-09: stale dev servers poison Playwright runs — reuseExistingServer:true + a server started under older code serves a stale module graph; symptoms are nondeterministic cross-run failure counts and absurd durations. Kill port 5173 + playwright processes before judging a "flaky" suite. Caught during V2 hardening pass.
- 2026-06-09: window-level key listeners + component-level key handlers on the same key need stopPropagation at the inner handler — activeElement guards can race synchronous Preact re-renders that unmount the focused element mid-dispatch. Caught by intermittent test failure during hardening.
- 2026-06-09: fire-and-forget async work after res.json() on Vercel serverless can be frozen before completing — await side effects (they can still swallow their own errors) before returning the response. Caught by Codex hardening review.
- 2026-06-09: live timer ticks need the same multi-segment catch-up chain as hydration — a backgrounded tab waking after several segment durations otherwise gifts the away-time back (startSegment anchors to now). Caught by Codex Phase 6 review.
- 2026-06-09: animation fill-mode 'both' leaves a terminal transform applied, creating a containing block that breaks position:fixed descendants — use 'backwards' for entry animations on wrappers that can contain fixed overlays. Caught by Codex motion review.
- 2026-06-09: session-generated records must stamp the actual wall-clock date (check `date` or git timestamps), not dates anchored from context documents — the V2 completion session backdated its entire close-out (handoff, PHASES.md current-state, five learning entries) to 16 May while running 9 June. Caught by next-session orientation cross-checking git commit timestamps against the handoff.
